...
void app_led_init(void) {
    osThreadDef(message, app_message_task, osPriorityAboveNormal, 0, 128);
    app_message_thread.id = osThreadCreate(osThread(message), NULL);
    /*msgq init*/
    osMessageQDef(message, 16, uint32_t);
    app_message_thread.queue = osMessageCreate(osMessageQ(message), NULL);
    /**event group init*/
    app_message_thread.event_group = xEventGroupCreate();

    osThreadDef(event, app_event_task, osPriorityNormal, 0, 128);
    app_event_thread.id = osThreadCreate(osThread(event), NULL);
    /*msgq init*/
    osMessageQDef(event, 16, uint32_t);
    app_event_thread.queue = osMessageCreate(osMessageQ(event), NULL);
    /* event group init*/
